...
Un Evidence Snapshot est une capture horodatée d'un élément observable dans la plateforme, conservée pour assurer l'auditabilité des observations. L'enquêteur peut les référencer par leur identifiant interne sans avoir besoin de connaître leur mécanique de stockage. Vous l'utilisez dans vos notes pour citer ce que vous avez vu, à quel moment, sans avoir à reproduire l'écran.
...
Plusieurs chaînes restent hors périmètre. Vous y rencontrerez à l'usage : la majorité des chaînes layer-2 émergentes après la date de cette version, les environnements orientés vers la confidentialité renforcée ou les flux à forte opacité, et les couches qui ne sont pas adressées par les explorateurs publics standards. Lorsqu'un signalement porte sur une chaîne hors périmètre, vous ne forcez pas un scan ; vous notez le périmètre manquant dans votre remontée et vous documentez l'enquête à partir des sources externes.
