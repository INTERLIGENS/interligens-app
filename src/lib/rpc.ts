...
export type RpcResult = { result: any; provider_used: string; cached: boolean };
...
export async function rpcCall(chain: "ETH" | "BSC" | "SOL", method: string, params: any[]): Promise<RpcResult> {
  ...
  if (hit && Date.now() - hit.t < TTL) return { result: hit.v.result, provider_used: hit.v.provider, cached: true };
  ...
  for (const url of endpoints) {
    try {
      ...
      return { result: data.result, provider_used: url, cached: false };
