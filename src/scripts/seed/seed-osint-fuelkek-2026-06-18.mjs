// Commit-time seeder for OSINT session 2026-06-18_fuelkek (KOL fuelkek).
// Idempotent: EvidenceSnapshot guarded by UNIQUE(sha256) via ON CONFLICT DO NOTHING;
//   KolTokenLink by UNIQUE(kolHandle,contractAddress,chain) via upsert;
//   KolAlias by UNIQUE(kolHandle,alias) via upsert;
//   KolWallet guarded by manual exists-check on (kolHandle,address,chain).
// This session: 0 wallets, 0 aliases (holdings auto-postes sans adresse -> note seule).
// NO migration (forensic columns already in prod via known drift).
// Usage: set -a; . ./.env.local; set +a; node src/scripts/seed/seed-osint-fuelkek-2026-06-18.mjs
import { PrismaClient, Prisma } from '@prisma/client'
import { readFileSync, existsSync } from 'node:fs'

const PLAN = 'exports/seed_plan_2026-06-18_fuelkek.json'
const REPORT = 'exports/seed_report_2026-06-18_fuelkek.json'
const plan = JSON.parse(readFileSync(PLAN, 'utf8'))
const prisma = new PrismaClient()
const report = { session: plan.session, startedAt: new Date().toISOString(),
  kolProfile: null, kolTokenLinks: { ok: 0, failed: [] },
  kolWallets: { inserted: 0, skipped_existing: 0, failed: [] },
  kolAliases: { ok: 0, failed: [] },
...
async function seedAliases() {
  for (const a of (plan.kolAliasesToCreate || [])) {
    try {
      await prisma.kolAlias.upsert({
        where: { kolHandle_alias: { kolHandle: a.kolHandle, alias: a.alias } },
        update: {},
        create: { kolHandle: a.kolHandle, alias: a.alias, type: a.type },
      })
      report.kolAliases.ok++
    } catch (e) { report.kolAliases.failed.push({ alias: a.alias, error: e.message }) }
  }
}

async function seedWallets() {
  for (const w of (plan.kolWalletsToCreate || [])) {
    try {
      const existing = await prisma.kolWallet.findFirst({
        where: { kolHandle: w.kolHandle, address: w.address, chain: w.chain } })
      if (existing) { report.kolWallets.skipped_existing++; continue }
      await prisma.kolWallet.create({
        data: { kolHandle: w.kolHandle, address: w.address, chain: w.chain,
          label: w.label, claimType: w.claimType, attributionSource: w.attributionSource,
          attributionNote: w.attributionNote, confidence: w.confidence },
      })
      report.kolWallets.inserted++
    } catch (e) { report.kolWallets.failed.push({ address: w.address, error: e.message }) }
  }
}
