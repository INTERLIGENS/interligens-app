...
// FR localized version of /en/watchlist/signals/[id]. Same logic, same
// API call, same render structure — only strings, date locale, and
// internal hrefs differ so the page stays consistently francophone when
// deep-linked in FR. Replaces the previous pure redirect to /en/.
...
const SIGNAL_TYPE_LABEL: Record<string, { label: string; color: string; desc: string }> = {
  ca_drop:             { label: 'CA DROP',       color: '#ef4444', desc: "Adresse de contrat postée directement" },
  ca_redirect:         { label: 'CA REDIRECT',   color: '#ef4444', desc: "Redirection vers la CA via pinned, bio ou DM" },
  nice_pump:           { label: 'PUMP BRAG',     color: '#f97316', desc: "Mise en avant de la performance du pump" },
  coded_bullish:       { label: 'CODED BULLISH', color: '#f59e0b', desc: "Langage bullish subtil (« has legs », « positioned »)" },
  coordination_signal: { label: 'COORDINATION',  color: '#ec4899', desc: "Langage suggérant une promotion coordonnée" },
  discovery_narrative: { label: 'DISCOVERY',     color: '#8b5cf6', desc: "Narratif de fausse découverte (« digging into », « noticed »)" },
  launch_language:     { label: 'LAUNCH',        color: '#f97316', desc: "Langage de lancement (presale, whitelist, fair launch)" },
  promo_language:      { label: 'PROMO',         color: '#f59e0b', desc: "Langage promotionnel générique" },
  emoji_signal:        { label: 'EMOJI SIGNAL',  color: '#6b7280', desc: "Motifs d'emojis à forte valeur détectés" },
}

const DEPTH: Record<string, { l: string; c: string }> = {
  comprehensive: { l: 'COMPLET', c: '#10b981' }, strong: { l: 'SOLIDE', c: '#3b82f6' },
  moderate: { l: 'MODÉRÉ', c: '#f59e0b' }, weak: { l: 'FAIBLE', c: '#6b7280' },
}
...
function scoreSeverity(score: number) {
  ...
  if (score >= 40) return { label: 'ÉLEVÉ', color: '#f97316' }
  if (score >= 25) return { label: 'MOYEN', color: '#f59e0b' }
  return { label: 'FAIBLE', color: '#6b7280' }
}

const timeAgo = (d: string | null) => {
  if (!d) return null; const diff = Date.now() - new Date(d).getTime(); const m = Math.floor(diff / 60_000)
  if (m < 60) return `il y a ${m} min`; const h = Math.floor(m / 60); if (h < 24) return `il y a ${h} h`
  const days = Math.floor(h / 24); if (days < 30) return `il y a ${days} j`; return `il y a ${Math.floor(days / 30)} mois`
}

const fmtDateFull = (d: string | null) => {
  if (!d) return ''; const dt = new Date(d)
  return dt.toLocaleDateString('fr-FR', { year: 'numeric', month: 'long', day: 'numeric' }) + ' à ' +
    dt.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })
}
...
export default function SignalDetailPage() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: '#000', color: '#f9fafb', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
        <div style={{ color: '#374151', fontSize: 12, fontFamily: 'monospace', letterSpacing: '0.15em' }}>CHARGEMENT DU SIGNAL...</div>
      </div>
    </div>
  )
  ...
  if (error || !signal) return (
    <div style={{ minHeight: '100vh', background: '#000', color: '#f9fafb', fontFamily: 'Inter, sans-serif' }}>
      <BetaNav />
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
        <div style={{ color: '#ef4444', fontSize: 14, fontWeight: 700, marginBottom: 8 }}>Signal introuvable</div>
        <a href="/fr/watchlist" style={{ color: '#F85B05', fontSize: 12, fontFamily: 'monospace' }}>Retour à la watchlist</a>
      </div>
    </div>
  )
    ...
    <div style={{ minHeight: '100vh', background: '#000', color: '#f9fafb', fontFamily: 'Inter, sans-serif', paddingBottom: 80 }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ marginBottom: 24, display: 'flex', alignItems: 'center', gap: 8 }}>
          <a href="/fr/watchlist" style={{ color: '#4b5563', fontSize: 11, fontFamily: 'monospace', textDecoration: 'none' }}>WATCHLIST</a>
        ...
        <div style={{ marginBottom: 24, padding: '16px 20px', background: '#0d1117', border: '1px solid #1e2330', borderRadius: 10, borderLeft: `3px solid ${sev.color}` }}>
          <div style={{ color: '#F85B05', fontSize: 9, fontWeight: 700, letterSpacing: '0.2em', fontFamily: 'monospace', marginBottom: 10 }}>TYPES DE SIGNAL DÉTECTÉS</div>
          {signal.signalTypes.length === 0 ? (
            <div style={{ color: '#374151', fontSize: 11, fontFamily: 'monospace' }}>Aucun type de signal spécifique classifié</div>
          ) : (
          ...
          <div style={{ marginBottom: 24, padding: '16px 20px', background: '#0d1117', border: '1px solid #1e2330', borderRadius: 10 }}>
            {signal.detectedAddresses.length > 0 && (
              <div style={{ marginBottom: signal.detectedTokens.length > 0 ? 12 : 0 }}>
                <div style={{ color: '#ef4444', fontSize: 9, fontWeight: 700, letterSpacing: '0.2em', fontFamily: 'monospace', marginBottom: 6 }}>ADRESSES DE CONTRAT DÉTECTÉES</div>
            ...
            {signal.detectedTokens.length > 0 && (
              <div>
                <div style={{ color: '#f59e0b', fontSize: 9, fontWeight: 700, letterSpacing: '0.2em', fontFamily: 'monospace', marginBottom: 6 }}>TOKENS MENTIONNÉS</div>
        ...
        <div style={{ marginBottom: 24, padding: '16px 20px', background: '#0d1117', border: '1px solid #1e2330', borderRadius: 10 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 24px' }}>
            ...
            <div>
              <div style={{ fontSize: 9, color: '#4b5563', fontFamily: 'monospace' }}>FOURNISSEUR</div>
              <div style={{ fontSize: 12, color: '#f9fafb' }}>{signal.sourceProvider}</div>
            </div>
            <div>
              <div style={{ fontSize: 9, color: '#4b5563', fontFamily: 'monospace' }}>DÉCOUVERT</div>
              <div style={{ fontSize: 12, color: '#f9fafb' }}>{fmtDateFull(signal.discoveredAt)}</div>
            </div>
            <div>
              <div style={{ fontSize: 9, color: '#4b5563', fontFamily: 'monospace' }}>POSTÉ</div>
              <div style={{ fontSize: 12, color: '#f9fafb' }}>{signal.postedAt ? fmtDateFull(signal.postedAt) : 'Inconnu'}</div>
            </div>
            {signal.profile?.followers && (
              <div>
                <div style={{ fontSize: 9, color: '#4b5563', fontFamily: 'monospace' }}>ABONNÉS AU MOMENT</div>
                <div style={{ fontSize: 12, color: '#f9fafb' }}>{signal.profile.followers.toLocaleString('fr-FR')}</div>
              </div>
            )}
          </div>
        </div>
        ...
        {kol && (
          <div style={{ marginBottom: 24, padding: '16px 20px', background: '#0d1117', border: '1px solid #1e2330', borderRadius: 10, borderLeft: '3px solid #F85B05' }}>
            <div style={{ color: '#F85B05', fontSize: 9, fontWeight: 700, letterSpacing: '0.2em', fontFamily: 'monospace', marginBottom: 10 }}>DOSSIER KOL — @{kol.handle}</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px 16px', marginBottom: 10 }}>
              {[
                ...
                { l: 'Gains', v: fmtUsd(kol.totalProceeds), c: '#ef4444' },
                { l: 'Preuves', v: String(kol.evidenceCount), c: '#3b82f6' },
                { l: 'Wallets', v: String(kol.walletsCount), c: '#f59e0b' },
                { l: 'Dossiers', v: String(kol.casesCount), c: '#8b5cf6' },
                { l: 'Profondeur', v: DEPTH[kol.evidenceDepth ?? '']?.l ?? '\u2014', c: DEPTH[kol.evidenceDepth ?? '']?.c ?? '#374151' },
              ].map(x => (
            ...
            {kol.isPublished && (
              <a href={`/fr/kol/${kol.handle}`} style={{ color: '#F85B05', fontSize: 11, fontFamily: 'monospace', fontWeight: 700, textDecoration: 'none' }}>
        ...
        {related.length > 0 && (
          <div style={{ marginBottom: 24 }}>
            <div style={{ color: '#6b7280', fontSize: 9, fontWeight: 700, letterSpacing: '0.2em', fontFamily: 'monospace', marginBottom: 10 }}>AUTRES SIGNAUX RÉCENTS DE @{signal.handle.toUpperCase()}</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {related.map(r => {
                const rs = scoreSeverity(r.signalScore)
                return (
                  <a key={r.id} href={`/fr/watchlist/signals/${r.id}`} style={{
        ...
        <div style={{ marginTop: 32, borderTop: '1px solid #1a1a1a', paddingTop: 20 }}>
          <a href="/fr/watchlist" style={{ color: '#F85B05', fontSize: 12, fontFamily: 'monospace', fontWeight: 700, textDecoration: 'none' }}>&larr; Retour à la watchlist</a>
        </div>
      </div>
    </div>
  )
