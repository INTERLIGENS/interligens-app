...
export const metadata: Metadata = {
  title: "LAB ($LAB) — 63M$ de dépôts pré-pump que le retail n'a jamais vus · INTERLIGENS",
  description:
    "Dossier Fraude Token IL-PND-LAB-001. Pump-and-dump / contrôle insider du supply sur BNB Chain. 68,75 % du supply LAB verrouillé dans 5 Gnosis Safes — vérifié on-chain de façon indépendante par INTERLIGENS.",
  openGraph: {
    title: "LAB ($LAB) — TigerScore 91 · AVOID · INTERLIGENS",
    description:
      "Dossier conjoint Specter × INTERLIGENS. Concentration de supply 68,75 % vérifiée on-chain.",
  },
};
...
export default async function LabCasePageFR() {
  const data = await getLab();
  if (!data) notFound();
  return <TokenCasefileView data={data} locale="fr" />;
}
