          ...
          <div className="flex gap-2 flex-wrap">
            {(["url","file","text"] as Tab[]).map(t => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition uppercase ${tab === t ? "bg-orange-500 text-black" : "bg-[#1a1a1a] text-gray-300 hover:bg-gray-700"}`}>
                {t}
              </button>
            ))}
          </div>
        ...
        {result && (
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-5 space-y-4">
            <div className="flex justify-between items-start">
              <div>
                <span className={`inline-block px-2 py-0.5 rounded text-xs bg-[#1a1a1a] font-semibold mr-2 ${classColor[result.classification] ?? "text-gray-400"}`}>
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                <div key={l as string} className="bg-[#1a1a1a] rounded-lg p-3 text-center">
            ...
            <div className="flex gap-2">
              <Link href={`/admin/intake/${result.intakeId}`} className="px-3 py-1.5 rounded-lg text-sm font-medium bg-[#1a1a1a] text-gray-300 hover:bg-gray-700 transition">
                View Record →
              </Link>
