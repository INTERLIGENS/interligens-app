...
async function main() {
  ...
  if (!sqlPath) {
    console.error("usage: tsx scripts/mm/applyMigration.ts <path/to/file.sql>");
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`[mm migration] applying ${statements.length} statements from ${sqlPath}`);
    ...
    console.log(`[mm migration] applied ${sqlPath}`);
  } finally {
    await client.end();
  }
}
