...
# Exceptions pour BUILD 6 / PACK C — persistance coordinated-exit. Deux modèles
# ADDITIFS sur le schema prod (ExitEvent, CoExitQualification), déclarés pour
# refléter un DDL exécuté À LA MAIN dans le Neon SQL Editor (verrou A9 : jamais
# db push, jamais prisma migrate). Le schema ne crée rien : il décrit.
# Autorisation humaine explicite (David, BUILD 6 / PACK C) — voir PR description.
# Exemption STRICTEMENT limitée à prisma/schema.prod.prisma ; AUCUN wildcard sur
# prisma/. La logique vit hors chemin gelé (src/lib/coordinated-exit/).
if [[ "$BRANCH" =~ ^feat/cc-offline-[0-9]+-coexit-f2-persistence$ ]]; then
    EXEMPT_COEXIT_PERSISTENCE_PATTERNS=(
        "^prisma/schema\\.prod\\.prisma$"
    )
fi
...
while IFS= read -r file; do
    ...
    # Sur la branche PACK C, exempter STRICTEMENT le schema prod (aucun wildcard
    # sur prisma/ : toute autre migration ou schema reste bloquée).
    if [[ "$BRANCH" =~ ^feat/cc-offline-[0-9]+-coexit-f2-persistence$ ]]; then
        EXEMPT=false
        for ex in "${EXEMPT_COEXIT_PERSISTENCE_PATTERNS[@]}"; do
            if [[ "$file" =~ $ex ]]; then
                EXEMPT=true
                break
            fi
