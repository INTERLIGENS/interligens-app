...
const TEMOIN: Impl = (engines) => {
  ...
  // ── CORRIGÉ EN S2 (T1) ──────────────────────────────────────────────────
  // La condition portait `&& base.verdict === "NO_CRITICAL_SIGNAL"` : le témoin
  // ne compensait QUE tant que `decide()` portait le défaut. Une fois celui-ci
  // fermé, le témoin cessait de produire son verdict de couverture, et le
  // mutant B4 — qui s'y accroche — cessait de muter.
  //
  // Un témoin ne doit pas dépendre de l'état du code qu'il sert à juger.
  // La règle gouvernée est inconditionnelle : zéro mesure ne rassure pas.
...
const MUTANTS: Mutant[] = [
  // ── Famille A : absence → réassurance ───────────────────────────────
  {
    ...
    // ── CORRIGÉ EN S2 (T1) ────────────────────────────────────────────────
    // Ce mutant déléguait au `decide()` RÉEL. Tant que celui-ci portait le
    // défaut, il mutait ; une fois le défaut fermé, il a cessé d'être un
    // mutant et le critère est passé sans rien prouver.
    //
    // Un mutant qui dépend du code sous test ne teste pas ce code : il le
    // recopie. Le verdict rassurant est donc désormais FIGÉ ici.
    impl: (e) =>
      e.filter((x) => x.ran).length === 0
        ? { ...TEMOIN(e), verdict: "NO_CRITICAL_SIGNAL" as ReflexVerdict }
        : TEMOIN(e),
  },
  {
...
// ═══ 4 · LA BATTERIE, APPLIQUÉE À L'IMPLÉMENTATION RÉELLE ════════════════
//
// Ajouté par T1 en S2. Sans ce bloc, S4 ne prouvait que son propre témoin et
// ses propres mutants : la batterie tournait à côté du code livré.
//
// Les 14 critères sont ici rejoués contre `decide()` — le vrai. C'est ce qui
// transforme la batterie en preuve de l'implémentation, et non en preuve
// d'elle-même.

describe("S4/4 — `decide()` réel ne viole aucun critère de la batterie", () => {
  it("zéro violation sur l'implémentation livrée", () => {
    expect(batterie(decide as unknown as Impl)).toEqual([]);
  });

  it("le 5e verdict porte son action, comme les quatre autres", () => {
    expect(ACTION_WORDING.INSUFFICIENT_COVERAGE.en.length).toBeGreaterThan(0);
    expect(ACTION_WORDING.INSUFFICIENT_COVERAGE.fr.length).toBeGreaterThan(0);
  });

  it("INSUFFICIENT_COVERAGE ne se déclenche QUE sur 0 mesuré", () => {
    // Le seul déclencheur gouverné. Aucun seuil entre 1/8 et 7/8.
    expect(decide(TOUS_EN_PANNE()).verdict).toBe("INSUFFICIENT_COVERAGE");
    expect(decide([]).verdict).toBe("INSUFFICIENT_COVERAGE");
    for (let n = 1; n <= 8; n++) {
      const m = [
        ...LES_HUIT.slice(0, n).map(propre),
        ...LES_HUIT.slice(n).map((e) => enPanne(e)),
      ];
      expect(decide(m).verdict).not.toBe("INSUFFICIENT_COVERAGE");
    }
  });
});
